...
export default function DeckPage() {
  ...
  useEffect(() => {
    // Support both ?client= (legacy share links) and ?clientId= (deck builder)
    const clientParam = searchParams.get("clientId") ?? searchParams.get("client");
  ...
  // Auto-generate when coming from Deck Builder (clientId param present + client loaded)
  const autoGeneratedRef = useRef(false);
  useEffect(() => {
    const fromBuilder = searchParams.get("clientId");
    if (!fromBuilder || autoGeneratedRef.current || !selectedClient || clientsLoading) return;
    autoGeneratedRef.current = true;
    generateDeck(selectedClient, selectedPeriod, searchParams.get("context") ?? undefined, undefined);
  }, [selectedClient, clientsLoading, searchParams, selectedPeriod, generateDeck]);
              ...
              >
                {clients.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name}
                    {c.platform === "google" ? ` (${c.googleCustomerId}) — Google Ads` : ""}
                    {c.platform === "both" ? ` — Meta + Google Ads` : ""}
                    {c.platform === "meta" ? ` — Meta Ads` : ""}
                  </option>
                ))}
              </select>
            )}
