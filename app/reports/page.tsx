...
import { mockReports, mockCreatives as _creatives, METRIC_LABELS, Report, ReportMetric } from "@/lib/mock-reports";
import { mockCreatives } from "@/lib/mock-data";
import { Share2, Copy, Eye, Plus, Check, ExternalLink } from "lucide-react";
...
function NewReportDialog({ onClose, onAdd }: { onClose: () => void; onAdd: (r: Report) => void }) {
  ...
  const toggleCreative = (id: string) =>
    setSelectedCreatives((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg mx-4 shadow-2xl">
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
              {mockCreatives.map((c) => (
                ...
                >
                  ...
                  <span className={`ml-auto text-xs font-semibold shrink-0 ${c.roas >= 3 ? "text-green-400" : c.roas >= 2 ? "text-yellow-400" : "text-red-400"}`}>
                    {c.roas}x
                  </span>
                </label>
              ))}
        ...
        <div className="p-5 border-t border-gray-800 flex gap-3 justify-end">
          ...
          <button
            ...
            disabled={!name.trim() || selectedCreatives.length === 0}
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm">Share performance data with your creative team — no login required</p>
        </div>
        <button
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">
                    ...
                    <span className={`font-semibold ${avgRoas >= 3 ? "text-green-400" : avgRoas >= 2 ? "text-yellow-400" : "text-red-400"}`}>
                      {avgRoas}x
                    </span>
                  </p>
